...
const Floor_1 = require("../entities/Floor");
...
const seedDatabase = async () => {
    ...
    const blockRepo = database_1.AppDataSource.getRepository(Block_1.Block);
    const floorRepo = database_1.AppDataSource.getRepository(Floor_1.Floor);
    const roomRepo = database_1.AppDataSource.getRepository(Room_1.Room);
    const bedRepo = database_1.AppDataSource.getRepository(Bed_1.Bed);
    const academicYearRepo = database_1.AppDataSource.getRepository(AcademicYear_1.AcademicYear);
    const applicationRepo = database_1.AppDataSource.getRepository(Application_1.Application);
    const preferenceRepo = database_1.AppDataSource.getRepository(ApplicationHostelPreference_1.ApplicationHostelPreference);
    const allocationRepo = database_1.AppDataSource.getRepository(Allocation_1.Allocation);
    const residentRepo = database_1.AppDataSource.getRepository(Resident_1.Resident);
    ...
    // 4. Seed Districts
    try {
        await database_1.AppDataSource.query("ALTER TABLE Districts ADD COLUMN IsAllowed TINYINT(1) NOT NULL DEFAULT 1");
    }
    catch (_colErr) {
